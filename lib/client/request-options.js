...
class RequestOptions {
  constructor(config, params) {
    this._config = config;
    this._requestOptions = this.createOptions(params);
  }
  ...
  createOptions(params) {
    ...
    if (this._config.options.authorizationMethod === authorizationMethodEnum.HEADER) {
      const encoding = new CredentialsEncoding(this._config.options.credentialsEncodingMode);
      const credentials = encoding.getAuthorizationHeaderToken(this._config.client.id, this._config.client.secret);
      ...
      parameters[this._config.client.idParamName] = this._config.client.id;
      parameters[this._config.client.secretParamName] = this._config.client.secret;
    }

    if (this._config.options.bodyFormat === bodyFormatEnum.FORM) {
  ...
  toObject(requestOptions = {}) {
    return Hoek.applyToDefaults(requestOptions, this._requestOptions);
  }
}
